...
def run_reclaim(*, password: str) -> None:
    """Prefer real `dbt run`; fall back to the identical T-SQL transform."""
    if shutil.which("dbt"):
        ...
        if proc.returncode == 0:
            print("  ✅ reclaim via `dbt run` (dbt-sqlserver)")
            return
        print("  ⚠️  dbt run failed, applying the model's T-SQL directly:")
        print("     " + (proc.stderr.strip().splitlines() or ["<no stderr>"])[-1])
    else:
        print("  ℹ️  dbt not on PATH — applying the model's identical T-SQL transform")
