{{
  config(
    materialized = "table",
    post_hook = [
      "CREATE CLUSTERED COLUMNSTORE INDEX cci_holdings_current ON {{ this }}"
    ]
  )
}}
...
-- (the post_hook). The driver drops the raw heap and SHRINKFILEs afterward, so
...
WITH ranked AS (
    ...
    FROM {{ source('loopcapital', 'holdings_snapshot_raw') }}
)
