...
(different owners, different model trees) and different job cadences. The BH-1330 receiver
(`platform-core syncProjectRuns`) and the brightbot `sync_project_runs` port composition never
branch on any of these — so the demo must actually exercise more than one project to show it.
(Cross-warehouse agnosticism is a related but separate claim, proven off LC's workspace — see
below.)
...
- [x] 🟢 Project 1 (Asset Management) linked to `loopcapital-dbt-demo` (verified #39)
- [ ] ⚪ Project 2 (Market Data) created + linked to its own repo + its own dbt Cloud job
- [ ] ⚪ Project 3 (Trade DW) created + linked to its own repo + its own dbt Cloud job
- [ ] ⚪ `syncProjectRuns` run once per project → each Observability tab fills from its own job
- [ ] ⚪ Fleet-health page shows LC's three projects with independent last-run / success-rate

**Throwaway/demo workspace (NOT LC) — cross-warehouse agnosticism:**
- [ ] ⚪ Student-scores Snowflake + Redshift + SSIS projects wired on a throwaway workspace
- [ ] ⚪ `syncProjectRuns` fills each identically → proof the path is warehouse-agnostic

## Guardrails

- The LC multi-project wiring stays on Loop Capital's real Snowflake — no invented second
  warehouse on their workspace. Adding projects 2–3 does **not** nuke anything: their single
  Asset-Management story stays intact until the deliberate nuke → rebuild step in
  [`DEMO_RUNBOOK_BEFORE_AFTER.md`](./DEMO_RUNBOOK_BEFORE_AFTER.md).
- The cross-warehouse (Redshift/SQL Server) story targets a **throwaway/demo workspace**, never
  Loop Capital's `e3fc0917-03a6-4ac6-aad4-ac265329bfb9` — those connections don't exist on LC's
  workspace and must never be presented as if they do.
