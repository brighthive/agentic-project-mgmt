...
class LoopCapitalSqlServerStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # SA password stored as a known shell-safe value rather than CDK-generated.
        # Generated passwords with special chars (!%][) get silently corrupted by
        # shell double-quote expansion in user-data -- SQL Server starts but SA
        # auth fails from external (root cause of first deploy failure). This is
        # demo infra (short lifetime, tagged TemporaryUntil); a known-strong-but
        # shell-safe password is the right tradeoff here.
        sa_secret = secretsmanager.Secret(
            self,
            "SqlServerSaSecret",
            description="Loop Capital demo SQL Server SA password - stored, never in source",
            secret_string_value=cdk.SecretValue.unsafe_plain_text(
                "LoopCapitalDemoSA2026Aa1"
            ),
        )
