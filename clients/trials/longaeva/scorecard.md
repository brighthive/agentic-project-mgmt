...
> - **GC-12 → `live` on staging** (behind kill-switches). Remaining before "fully live to the client": (1) flip the webapp runtime flag once the Amplify build lands; (2) wire the **scheduled** trigger to actually run the capability on a cadence (the dispatcher exists; honoring `applyOnSchedule` is BH-670, small); (3) BrightSignals anomaly push (BH-671 read-path done; Slack sink is GC-13). The detect→snapshot→persist→read core is proven.
...
>
> **cycle-23 completeness: ~46%** weighted (live=1.0 × 4 + partial=0.5 × 4 = 6.0 / 13 = 46%); **strict fully-live: 31%** (4/13). GC-12's remaining edges (scheduled-cadence trigger BH-670, BrightSignals anomaly sink) are enhancements, not blockers — the detect→persist→read loop is proven live. No overclaim: GC-12 counts as live because it was tested end-to-end on staging, not because code merged.
>
