...
author: kuri
last_reviewed: "2026-06-16-cycle-20"
amended:
  - "2026-06-16 cycle-20 — Atlas semantic-view READ path closed (BH-624 epic shipped to staging). Trigger: real user question on staging — 'what are the semantic ymls of those tables?' — hit a dead end because BrightAgent had only WRITE support (BH-619/620/622) and no read tool. Closed end-to-end across 4 PRs (brightbot #552 → develop, #553 → staging release, #554 → develop docs, #555 → staging docs release; platform-saas-ai-context #18) in ~3 hours with 3 rounds of multi-agent review. Tickets closed: BH-625 (`SnowflakeConnection.get_semantic_view_ddl()` via `GET_DDL`), BH-626 (`get_semantic_view` @tool — Snowflake DDL by table name), BH-628 (wired into retrieval/dbt/metadata agents), BH-629 (deep_agent prompt routing), BH-630 (16 unit tests), BH-631 (5 live LONGAEVA_POC integration tests), BH-632 (12 prompt-surface routing scenarios), BH-633 (`get_semantic_view_yaml` @tool — byte-identical Atlas YAML from Platform Core's `DataAsset.semanticViewYaml`, preserves `atlas:*` / `owners` / `dataset_key` / `verified_queries`), BH-640 (GitHub fetch — merged into BH-633 after analysis showed they converged on the same field), BH-641 (Stage 4 deploy — closed as done-by-existing-tools; `ship_semantic_view_to_github(auto_merge=True)` triggers Atlas SDK on Longaeva's side per spec; BrightHive emits YAML only, never DDL). Live verdict: 117 unit tests + 5 LONGAEVA_POC integration tests + 2 staging integration tests, all green. Three review rounds caught a merge-blocker (`get_data_assets_by_workspace_id` was missing the new fields → silent 'no YAML' regression), promoted `__platform_client__` magic string to a `Final` constant, hardened error-envelope handling (data:null / GraphQL errors[] / data:list / workspace:list all distinct), removed PII from chat (asset names → logger only). Cross-repo: `platform-saas-ai-context` capability-map.md updated (PR #18, merged) — flipped SV compilation/validation row from Sprint→Shipped, added 3 new rows for read tools + deploy. New deferral: BH-645 (code-level HITL gate on `auto_merge=True`; safety is prompt-only today). Lifecycle doc: `brightbot/docs/SEMANTIC_VIEW_LIFECYCLE.md`."
  ...
  - "2026-06-09 cycle-17 FINAL — discoverability + CI gate + filler closure. Cycles 8-17 shipped 9 more PRs (5 substantive, 4 pure FILLER marked as such in PR body). Substantive: pc#804 (`make verify` + `make verify-pristine` cold-start one-command pre-flight, drive-by fix to make-help regex), pc#805 (CI workflow `unit-tests.yml` gating 70 unit tests on every PR to develop/staging/main — locks Properties 1-4 + scrub-bearer + URL parser; tests/setup.ts gets `SKIP_DEV_SERVER_CHECK=1` escape hatch), agentic-project-mgmt#31 (scorecard + GAPS amended for cycle-7 state — predecessor to this entry), agentic-project-mgmt#32 (clients/trials/longaeva/README.md folder index — single entry point for new joiners), brighthive-scripts#5 (README expanded from 1-line stub to full repo index). Filler: pc#806 (Unit Tests CI badge on README), agentic-project-mgmt#33 (root README clients/ section), agentic-project-mgmt#34 + brighthive-scripts#6 (CONTRIBUTING.md per repo), agentic-project-mgmt#35 (runbook glossary expansion to 30 entries with errorCode taxonomy keyed to recovery paths). Originally-planned brightbot CHANGELOG.md entry was abandoned mid-cycle on discovery that release.yml runs generate_release_notes.py --write on push to main — would have collided. Eng channel updated 2026-06-09 (#engineering ts 1781011761.012769). **Final tally: 20 merged PRs across 4 repos** (platform-core 9, agentic-project-mgmt 6, brighthive-scripts 4, brightbot 1). Plus pc#798 (SPEC-SEMANTIC-VIEW-AUTHORING-E2E open draft, awaiting §10 sign-off). **Trial-Day-1 platform readiness: ✅ green**. `make verify-pristine` opens a real GitHub PR in ~30s on a fresh laptop. **Outstanding for Day 1 unchanged**: humans-only items (staging deploy, BH-533 connectivity, demo storyboard with Grant, LONGAEVA_AGENT_ROLE, customer creds). **Sprint-sized deferrals unchanged**: bb#489, generate_mart_model, Snowflake auto-trigger, BH-615+BH-616 webapp UI."
  - "2026-06-08 cycle-7 — POST-MERGE-TRAIN AUTONOMOUS LOOP (8 more PRs across 4 repos). pc#797 (auth role hierarchy fix BH-612 + 7 unseeded OGM types + 109-mutation inventory), pc#798 (SPEC-SEMANTIC-VIEW-AUTHORING-E2E open draft), pc#799 (BH-613 WorkspaceGitHubBindingNode + setWorkspaceGitHubBinding), pc#800 (BH-614 commitSemanticViewToGitHub orchestrator — 9-step pipeline, idempotent retry, verified against real github.com/brighthive/longaeva-semantic-views), pc#801 (BH-618 20 deterministic eval tests for Properties 1-4), pc#802 (BH-611 LocalStack in compose + endpoint-aware AWS clients — local stack fully self-sufficient), pc#803 (audit-debt #11 token redaction extended to ghu_*, ?access_token=, &pat= — 20 regression tests), bb#520 (audit-debt #10 GHE proxy selection-set drift fixed — 7 mutations now select errorCode + httpStatus, revives BRANCH_EXISTS retry path), brighthive-scripts#3 (BH-617 provision_semantic_views_repo.sh), brighthive-scripts#4 (trial_day_1_dry_run.sh — 5-step demo smoke, verified happy + auto-merge + bad-JWT paths), agentic-project-mgmt#30 (OPERATOR-RUNBOOK-DAY-1.md — pre-flight + 4-mutation live script + 7 named recovery paths). **GC-6 platform layer fulfills its purpose end-to-end**: anyone with PC_GRAPHQL_URL + admin JWT + workspace/asset UUIDs can run scripts/trial_day_1_dry_run.sh and a real GitHub PR opens in ~30s. Property 1 (PAT redaction) and Property 2 (yamlHash continuity) hold under test. **Sprint-sized deferrals (intentional)**: bb#489 multi-table semantic view, generate_mart_model (GC-5), Snowflake auto-trigger (~50 lines pc + SSM), BH-615 + BH-616 (webapp UI — paused per 'no UI mess' instruction). **Humans-only outstanding**: staging deploy of pc#797-803 + bb#520, BH-533 connectivity validation, demo storyboard with Grant, LONGAEVA_AGENT_ROLE runtime in CDK."
status: trial-active-cycle-20
---
