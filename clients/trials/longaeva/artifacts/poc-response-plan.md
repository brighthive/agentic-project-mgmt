...
| 2.1 | Semantic view YAML from Silver table + plain-lang description: dims, time-dims, facts, metrics, Atlas custom blocks | LLM-driven YAML generation against the **Atlas YAML contract** (received from client 2026-06-01 — `artifacts/atlas-semantic-view-spec.md`). | Engineering | **Sprint to close** | New semantic view scaffolding tool (BH-531, 5 pts). Contract is concrete — Atlas SDK strips `atlas:*` keys before DDL, so BrightHive emits Atlas-shaped YAML, never DDL. |
| 2.2 | Reference data binding: fiscal calendar / LEI-FIGI→`LNGV_ISSUER_ID` / geographic codes | **Atlas-target binding** when Silver columns match known identifiers (Atlas inherits parallel columns populated upstream in dbt — not runtime joins). | Eng + KG | **Ready** | Inference table in distilled spec maps column patterns → `atlas.target` (e.g. `LNGV_*` → `lngv_issuer_id`, `BLOOMBERG_TICKER` → `bloomberg_ticker`, `GEOGRAPHY` → `metric_attributes.geography.*`). |
| 2.3 | Validate semantic view via end-to-end query through MCP | Atlas SDK owns DDL generation; BrightHive validates by running a `verified_query` through `SnowflakeConnection` (BH-527 merged) + MCP. | Quality + Eng | **Sprint to close** | No more `CREATE SEMANTIC VIEW` execution path — Atlas handles that. We validate by query round-trip. Depends on MCP client config (BH-532). |
...
Four agent-orchestrated steps, **grounded in the Atlas YAML contract** (`artifacts/atlas-semantic-view-spec.md` — distilled from client examples received 2026-06-01):

1. **Schema ingestion**: Supervisor reads Silver table schema via `SnowflakeConnection` (BH-527 merged) + plain-language description
2. **YAML scaffolding**: Engineering Agent generates Atlas-shaped YAML — `name` (SVW__ pattern), `description`, `custom_instructions`, `tables[]` with `base_table`/`primary_key`/`dimensions`/`time_dimensions`/`facts`/`filters`, `verified_queries[]` (in Snowflake `SEMANTIC_VIEW(...)` syntax), and the `atlas` top-level block (`dataset_key`, `entities.primary`, `defaults`, `dagster_dep`, `owners`). The example library (3 real semantic views) is loaded into workspace context as the LLM's stylistic reference.
3. **Atlas-target binding**: Engineering Agent binds `atlas.target` from column-name inference (see distilled spec §6 mapping table — `LNGV_*` → `lngv_issuer_id`, `BLOOMBERG_TICKER` → `bloomberg_ticker`, `GEOGRAPHY` → `metric_attributes.geography.*`, etc.). The KG holds the inference table; no runtime joins are needed because Atlas inherits parallel columns from upstream dbt.
4. **Validation + PR**: Quality Agent validates by running a `verified_query` end-to-end through MCP (Atlas SDK owns DDL generation — BrightHive does not emit `CREATE SEMANTIC VIEW`). Raises GitHub Enterprise PR on success.

**Context setup is the prerequisite**: enrollment accuracy depends on (a) Atlas example library loaded (✅ done), (b) the `atlas.target` inference table loaded (✅ in spec), and (c) Longaeva's production naming conventions confirmed (Day 3).
...
|---|---|---|---|
| **Atlas YAML contract example library** | LLM's stylistic + structural reference for scaffolding | Longaeva | ✅ Day 0 — `artifacts/atlas-semantic-view-examples.yaml` |
| **Atlas YAML distilled spec + inference table** | Engineering Agent's inference rules for `atlas.target`, fact aggregation defaults, naming conventions | Brighthive | ✅ Day 0 — `artifacts/atlas-semantic-view-spec.md` |
| Fiscal calendar reference table schema + sample rows | Pattern for `atlas.target` binding when fiscal-alignment columns appear | Longaeva | Day 3 |
| LEI / FIGI → `LNGV_ISSUER_ID` mapping (output already in Silver as parallel columns) | Confirms inference: `LNGV_*` → `atlas.target=lngv_issuer_id`; `BLOOMBERG_TICKER` → `atlas.target=bloomberg_ticker` | Longaeva | Day 3 |
| Existing dbt project structure + conventions (sanitized examples show `INT__<DOMAIN>__<SUBJECT>`) | Generated artifacts match their conventions | Longaeva | Day 3 |
| Dagster asset graph (OpenLineage export or direct connection) | Observability Agent reads lineage; **Atlas YAML only requires the `dagster_dep` tuple** — no execution wiring needed | Longaeva | Day 3-4 |
| Sample vendor datasets (one per source type) | Ingestion Agent reads schema to generate artifacts | Longaeva | Day 4-5 |
| Production semantic view definitions (un-sanitized analogues of the 3 examples) | Validate scaffolding tool against real production naming | Longaeva | Day 4-5 |
| Brighthive workspace context file | Stack conventions, terminology, Atlas inference table, reference data glossary | Brighthive | Day 4-5 |
