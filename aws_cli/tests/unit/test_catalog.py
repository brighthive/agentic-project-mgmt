...
def test_catalog_persistence_with_secret_values():
    with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as tmp:
        catalog_path = Path(tmp.name)
        ...
        secret = AWSSecret(
            ...
            source=SecretSource.SECRETS_MANAGER,
            secret_value={"host": "db.example.com", "password": "s3cret"},
        )

        catalog1 = SecretsCatalog(catalog_path=catalog_path)
        catalog1.add(secret=secret)
        catalog1.save()

        catalog2 = SecretsCatalog(catalog_path=catalog_path)
        retrieved = catalog2.get(secret_id="arn:val")

        assert retrieved is not None
        assert retrieved.secret_value == {"host": "db.example.com", "password": "s3cret"}
        assert retrieved.secret_value["password"] == "s3cret"
