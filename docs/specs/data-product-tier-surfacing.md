...
- `PipelineTier { RAW, BRONZE, SILVER, GOLD, PLATINUM, UNKNOWN }` is a real enum
  (`brighthive-platform-core/src/graphql/ogm/pipeline-typedefs.ts:15`), carried on every
  `DataAssetNode.pipelineTier` (`typedefs.ts:567`) and `TransformationNode.pipelineTier`, **derived**
  from the longest `DERIVES_FROM` chain and cached (`service/neo4j/pipeline-lineage.ts:444-479`).
...
- The **Created Data Products grid** (`src/Projects/CreatedDataProductsPage/CreatedDataProductsGrid.tsx:45`)
  ...
  (`schema.graphql:1228-1243`) never selects `pipelineTier`, though the underlying node carries it.
...
|---|---|---|
| **W1** | Tier badge column on the Created Data Products grid (additive `pipelineTier` on `CreatedFinalProduct` + column) | Smallest, highest-signal — one field + one column over data that already ships |
| **W2** | Gold/Platinum **filter** on the grid, driving the existing `pipelineLineage(tier:)` query | Answers "show me only trusted products" with an already-implemented query arg |
...
```graphql
# schema.graphql — CreatedFinalProduct gains ONE additive, nullable field.
# Value is READ from the node's already-derived DataAssetNode.pipelineTier — never set here.
type CreatedFinalProduct {
  # ...existing fields (schema.graphql:1228-1243) unchanged...
  pipelineTier: PipelineTier   # nullable: UNKNOWN/absent for a product with no resolved lineage
}
```

Resolver: `ProjectModel.getCreatedDataProducts` (`src/graphql/models/project.ts:1096`) already loads each
product's terminal `DataAssetNode`; it returns that node's existing `pipelineTier`. No new derivation, no
Cypher change — the tier is already computed and cached on the node.

### 2.2 W2 — grid tier filter (webapp, over the existing query)

```typescript
// src/Projects/CreatedDataProductsPage/ — filter state over the ALREADY-IMPLEMENTED
// pipelineLineage(workspaceId, tier: PipelineTier) query (pipeline-lineage-typedefs.ts:135).
// "Trusted" preset = { GOLD, PLATINUM }. No new backend query.
...
- `PipelineTier` enum + derived `pipelineTier` field (platform-core, ships) — read-only source of truth.
- `pipelineLineage(tier:)` query (`pipeline-lineage-typedefs.ts:135`, implemented) — W2 filter driver.
- `ProjectModel.getCreatedDataProducts` (`project.ts:1096`) — W1 resolver already loads the node.
...
3. **webapp** — Gold/Platinum filter over `pipelineLineage(tier:)`. (S) — **W2**
...
| BH-XXXX (to create) | `feat(webapp): Gold/Platinum tier filter over the existing pipelineLineage(tier:) query` | W2 | S |
