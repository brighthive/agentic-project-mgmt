...
3. **Not a fleet-monitored source.** Quality evaluation isn't a `PipelineSource` (`pipeline_health.py:86-95`), so it can't ride the fleet sweep, fairness, resilience, or the routine-scheduling seam ([`brightroutine-approve-schedule.md`](./brightroutine-approve-schedule.md)). It runs only inside the quality agent's own tools, on demand.
