...
# NOTE: this spec covers only the EXECUTION surface of DbtCoreRunner (run_segment/run_on_ref
# against an existing dbt project ref). The FULL DbtCoreRunner lifecycle contract — capabilities(),
# provision_isolated/teardown, set_schedule, and the observe verbs — is owned by
# autonomous-dbt-project-lifecycle.md (BH-1403). Signatures below mirror the real port verbatim:
# brightbot/pipelines/core/port.py returns RunHandle (line 45), takes LineageSegment (line 37).
class DbtCoreRunner(PipelineRunner):  # implements brightbot/brightbot/pipelines/core/port.py Protocol
    ...
    async def run_on_ref(self, *, pipeline_id: str, git_ref: str, ctx: RequestContext) -> RunHandle: ...
```
...
THEN it SHALL select DbtCoreRunner, never DbtCloudRunner
  Response (typed): RunHandle | RunnerCapabilityError{reason: "dbt_cloud_unsupported_target" | "dbt_core_unavailable"}
```
...
- `DbtCoreRunner` SHALL execute only the validated `dbt` subcommand + allowlisted flags passed
  through the `LineageSegment`/`git_ref` contract — no shell interpolation of untrusted strings
...
Feature: On-premise SQL Server warehouse support
  ...
  Scenario: dbt Core runner executes a real build against the EC2 stand-in
    ...
    Then the dbt build completes and the returned RunHandle reports success with real row/model counts
...
*For any* `LineageSegment`/`git_ref` value passed to `DbtCoreRunner.run_segment`/`run_on_ref`, the
