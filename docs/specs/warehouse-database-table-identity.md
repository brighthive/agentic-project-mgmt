...
>   context) needs a stable address to attach to. §2a's `TableResourcePort`
...
class TableResourcePort(Protocol):
...
Levels 4 (resource) and 5 (job) existed only as Python-side port contracts (§2a `TableResourcePort`,
§2f `WarehouseAdminPort`) with no Neo4j representation — so a query like "show every resource and
...
### 2f. WarehouseAdminPort — the write/DDL capability (NEW, closes the Gaps entry above)

A second, distinct port from `TableResourcePort` (§2a) — administrative
...
class WarehouseAdminPort(Protocol):
...
**Why this is a new port, not an extension of `TableResourcePort` or a flag on
...
write-capable via a parameter. Every `WarehouseAdminPort` call requires an
...
PROCEDURE / upsert sequence becomes one `resolve_warehouse` → `WarehouseAdminPort`
...
| — | brightbot: document (not implement) `TableResourcePort` registry seam for schema/profiler/sessions/lineage/embeddings | 2 | BH-NEW |
...
| — | (6, added 2026-08-06) brightbot: `WarehouseAdminPort` (§2f) — `create_database`/`create_table`/`create_procedure`/`upsert_rows`, first adapter `SynapseAdminAdapter`; every call writes a `JobNode` row (status + reason) so provisioning itself is lineage-visible, not a side-channel script | 8 | BH-NEW |

**Total: 93 points across 23 tickets**
