...
| Area | Repo | Impact |
|------|------|--------|
| Platform Core | `brighthive-platform-core` | New `DatabaseNode`, new relationships, `isDefault` field + constraint, WorkspaceAdmin-gated mutations/queries, derived + rename-cascaded `tableFQN`, cascade-flag-gated delete, ordered data migration for existing warehouses; OM `DatabaseService` naming fix (one identity per warehouse connection, not per workspace+provider-type) |
...
**Total: 77 points across 20 tickets**
