...
as a registry seam now; **only the port and the 5 concrete adapters below ship
...
| Area | Repo | Impact |
|------|------|--------|
| Platform Core | `brighthive-platform-core` | New `DatabaseNode`, new relationships, `isDefault` field + constraint, WorkspaceAdmin-gated mutations/queries, derived + rename-cascaded `tableFQN`, cascade-flag-gated delete, ordered data migration for existing warehouses |
| BrightBot | `brightbot` | Replace `next(iter(...))`/`[0]` warehouse+table resolution with `isDefault`-aware + ambiguity-ladder resolvers; always disclose warehouse/database used; multi-database connection/introspection support; §2a resource port for the 5 concrete resource kinds |
| Web App | `brighthive-webapp` | Settings → Warehouses: default badge + toggle. New Data Assets → Databases page (warehouse → database → table browser) |
...
**Total: 54 points across 16 tickets**
