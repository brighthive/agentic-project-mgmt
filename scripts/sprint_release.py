...
# Create SSL context - try certifi first, fall back to unverified for dev
try:
    import certifi
    SSL_CONTEXT = ssl.create_default_context(cafile=certifi.where())
except ImportError:
    # Fallback: create unverified context (OK for internal dev tools)
...
def print_validation_report(report: ValidationReport) -> None:
    ...
    if report.missing_tickets:
        print("⚠ DONE TICKETS WITHOUT PR (needs investigation):")
        for t in report.missing_tickets:
            print(f"  - {t.key}: {t.summary[:50]}... ({t.ticket_type})")
        print()

    # No code expected tickets (informational)
    if report.no_code_tickets:
        print("ℹ TICKETS NOT REQUIRING CODE VALIDATION:")
        by_status: dict[str, list[Ticket]] = {}
        for t in report.no_code_tickets:
...
def main() -> None:
    ...
    sprint_config = {
        ...
        5: ("Sprint 5 🍋", "2026-02-10", "2026-02-17"),
    }
